...
class UpdateHelper(DateChecker):
  ...
  def __init__(self, website: dict, **kwargs):
    ...
    self.for_section_update = True if any([(two_weeks and is_weekend), not sections]) else False

    self.for_article_update = True if all([not self.is_today(self.date_updated), not self.for_section_update]) else False